...
def welcome():
    print("""
    Welcome to Polaris Banking App. Which of the following operations would you like to perform?
...
def registration():
    myquery = "INSERT INTO polaris(first_name, last_name, user_name, age, address, occupation, account_type, account_number, account_balance, phone_number, network, passwd) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    if account_type == "1":
        ...
        account_type = "Savings"
    checkPassword()
    account_number = random.randrange(245090001, 245098999)
    values = (first_name, last_name, email, age, address, occupation, account_type, account_number, 0, phone_number, network, password)
    cursor.execute(myquery, values)
    myconn.commit()
    query = "SELECT * from polaris where passwd =%s and user_name =%s"
    val = (password, email)
    cursor.execute(query, val)
    result = cursor.fetchone()
    if result:
        time.sleep(2)
        print(f"Hello {result[1]}, your registration is successful and your account number is {result[8]}")
...
def login():
    ...
    try:
        ...
        if login_username == login_result[3] and login_password == str(login_result[12]):
            mainMenu()
    except TypeError:
...
def checkBalance():
    ...
    cursor.execute(query, val)
    login_result = cursor.fetchone()
    print(f"Your account balance is #{login_result[9]}.00 naira")
        ...
        if result == None:
            ...
            try:
                ...
                if balance < amount:
                    ...
                    myquery = "UPDATE polaris SET account_balance =%s where id =%s"
        ...
        if subscription == "1":
            ...
            if response == "y":
                ...
                if login_result[9] < basic:
                    ...
                    new_amount = login_result[9] - basic
                    val2 = (new_amount, login_result[0])
                    myquery = "UPDATE polaris SET account_balance =%s where id =%s"
                    cursor.execute(myquery, val2)
                    myconn.commit()
            ...
            if response == "y":
                ...
                if login_result[9] < regular:
                    ...
                    new_amount = login_result[9] - regular
                    val2 = (new_amount, login_result[0])
                    myquery = "UPDATE polaris SET account_balance =%s where id =%s"
                    cursor.execute(myquery, val2)
                    myconn.commit()
            ...
            if response == "y":
                ...
                if login_result[9] < compact:
                    ...
                    new_amount = login_result[9] - compact
                    val2 = (new_amount, login_result[0])
                    myquery = "UPDATE polaris SET account_balance =%s where id =%s"
                    cursor.execute(myquery, val2)
                    myconn.commit()
            ...
            if response == "y":
                ...
                if login_result[9] < premium:
                    ...
                    new_amount = login_result[9] - premium
                    val2 = (new_amount, login_result[0])
                    myquery = "UPDATE polaris SET account_balance =%s where id =%s"
                    cursor.execute(myquery, val2)
                    myconn.commit()
...
def reset():
    ...
    if not password.isdigit() or len(password) <= 3:
        ...
        if password == confirm_password:
            ...
            myquery = "UPDATE polaris SET passwd =%s where id =%s"
            cursor.execute(myquery, val2)
            myconn.commit()
            print("Please wait")
            time.sleep(2)
