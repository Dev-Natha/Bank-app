...
def welcome():
    print("""
    Welcome to Zenith Banking App. Which of the following operations would you like to perform?
...
def registration():
    myquery = "INSERT INTO zenith(first_name, last_name, user_name, age, address, occupation, account_type, account_number, account_balance, phone_number, network, passwd) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    ...
    if account_type == "1":
        ...
        account_type = "Savings"
    checkPassword()
    account_number = random.randrange(312870001, 312878999)
    ...
    myconn.commit()
    query = "SELECT * from zenith where passwd =%s and user_name =%s"
    val = (password, email)
        ...
        if result == None:
            ...
            try:
                ...
                if balance < amount:
                    ...
                    myquery = "UPDATE zenith SET account_balance =%s where id =%s"
...
def reset():
    ...
    if not password.isdigit() or len(password) <= 3:
        ...
        if password == confirm_password:
            ...
            myquery = "UPDATE zenith SET passwd =%s where id =%s"
            cursor.execute(myquery, val2)
            myconn.commit()
            print("Please wait")
            time.sleep(2)
